...
#' @param washoutPeriodDays                     The number of days after entering target cohort to consider subject "safe" and eligible for comparator cohort.
#' @param highRiskGroupDefinition               A list to pass to the `filterPersonsAgainstHighRiskGroupDefinition()` defining which subjects are considered high risk.
#' @param matchHighRiskCriteria                 A value of `any` (has any high risk characteristic) or `all` (has all defined high risk characteristics) to
#'                                              define how to match high risk subjects.
#' @param lowRiskComparatorIncidence            The incidence rate of the outcome in the low risk comparator group (those who do not match the high risk criteria).
#' @param lowRiskTargetIncidence                The incidence rate of the outcome in the low risk target group (those who do not match the high risk criteria).
#' @param highRiskComparatorIncidence           The incidence rate of the outcome in the high risk comparator group (those who match the high risk criteria).
#' @param highRiskTargetIncidence               The incidence rate of the outcome in the high risk target group (those who match the high risk criteria).
...
#' @param allOutcomes   A tibble containing the outcomes for the matched cohort generated by the `getOutcomesForMatchedCohort` function.
...
#' `studyEndDate.`
#'
#' @param cohort            A data frame containing the cohort table generated by the `generateSimulatedCohortTable` function.
#' @param matchedCohort     A data frame containing the matched cohort table generated by the `generateMatchedCohort` function.
#' @param targetId          The target cohort definition ID.
#' @param outcomeIds        A vector of outcome cohort definition IDs.
#' @param daysFromObsStart  The number of days after the target cohort start date to begin the risk window for adverse events.
#' @param daysToObsEnd      The number of days after the target cohort start date to end the risk window for adverse events.
#' @param studyEndDate      The date to use as the end of the study period. If specified, outcomes that occur after this date are excluded.
#'
#' @return
...
#'
#' @param targetCohort      A target cohort data frame generated by `generateTargetCohortFromSimulatedCohortTable` function.
#' @param comparatorCohort  A comparator cohort data frame generated by `generateComparatorCohortFromSimulatedTargetCohortTable` function.
#' @param matchedStrata     A matched strata data frame generated by `generateMatchedStrataForComparatorCohort` function.
#' @param persons           A persons data frame generated by `generateSimulatedPersonsData` function.
#'
#' @return
...
#'
#' @description
#' This function takes a cohort, exposureId, strata data frame, and persons data frame and returns a person-level
#' dataset that can be used for analysis.
#'
#' @param cohort     A target cohort data frame generated by `generateTargetCohortFromSimulatedCohortTable` function or
#'                   a comparator cohort data frame generated by `generateComparatorCohortFromSimulatedTargetCohortTable` function.
#' @param exposureId The exposure ID for the cohort.
#' @param strata     A matched strata data frame generated by `generateMatchedStrataForComparatorCohort` function.
#' @param persons    A persons data frame generated by `generateSimulatedPersonsData` function.
#'
#' @return
#' A tibble with the following columns:
...
getPersonLevelDataFromCohortAndStrata <- function(cohort, cohortType, strata, persons) {
    ...
    cohort %>%
        ...
        select(cohort_definition_id, subject_id, cohort_start_date, cohort_end_date, age_group_id, gender_concept_id, race_concept_id, ethnicity_concept_id) %>%
        inner_join(strata, by = c("cohort_definition_id", "cohort_start_date", "age_group_id", "gender_concept_id", "race_concept_id", "ethnicity_concept_id")) %>%
        mutate(
            exposure_id = ifelse(cohortType == 'target', 1, 0),
        ) %>%
        select(cohort_definition_id, exposure_id, strata_id, subject_id, cohort_start_date, cohort_end_date, age_group_id, gender_concept_id, race_concept_id, ethnicity_concept_id)
}


#' Gets unique strata from a cohort and persond data frame.
#'
#' @param cohort    A cohort data frame with cohort_definition_id, cohort_start_date
#' @param persons   A persons data frame generated by `generateSimulatedPersonsData` function.
#'
#' @return
#' A data frame with the following columns:
...
generateStrataFromCohort <- function(cohort, persons) {
    cohort %>%
        ...
        select(cohort_definition_id, cohort_start_date, age_group_id, gender_concept_id, race_concept_id, ethnicity_concept_id) %>%
        distinct(cohort_definition_id, cohort_start_date, age_group_id, gender_concept_id, race_concept_id, ethnicity_concept_id) %>%
        mutate(
            strata_id = rownames(.)
        ) %>%
        select(cohort_definition_id, strata_id, cohort_start_date, age_group_id, gender_concept_id, race_concept_id, ethnicity_concept_id) %>%
...
#' in the provided strata data frame. This has effect of remove strata from the provided strata
#' that do not exist in the comparator cohort.
#'
#' @param strata            A strata data frame generated by `generateStrataFromCohort` function.
#' @param comparatorCohort  A comparator cohort data frame generated by `generateComparatorCohortFromSimulatedTargetCohortTable` function.
#' @param persons           A persons data frame generated by `generateSimulatedPersonsData` function.
#'
#' @return
...
generateMatchedStrataForComparatorCohort <- function(strata, comparatorCohort, persons) {
    generateStrataFromCohort(comparatorCohort, persons) %>%
        ...
        inner_join(
            strata,
            by = c("cohort_definition_id", "cohort_start_date", "age_group_id", "gender_concept_id", "race_concept_id", "ethnicity_concept_id")
        ) %>%
        select(cohort_definition_id, strata_id, cohort_start_date, age_group_id, gender_concept_id, race_concept_id, ethnicity_concept_id)
}
...
#'
#' @return
#' A numeric value corresponding to the bin number a person would fall in based on their age, with bins of 5 years starting at birth.
getAgeGroupIdForReferenceDateAndDateOfBirth <- function(referenceDate, dateOfBirth) {
    cut(as.numeric(difftime(as.Date(referenceDate), as.Date(dateOfBirth), units = 'days')) / 365.25, seq(0, 100, 5), labels=FALSE)
}
...
#'
#' @param cohort                            A data frame generated by the `generateSimulatedCohortTable` function.
...
#'
#' @param targetCohort                       A data frame generated by the `generateTargetCohortFromSimulatedCohortTable` function.
#' @param riskWindowAfterTargetExposureDays  The number of days after the target exposure to include in the risk window.
...
#'
#' @param persons         A data frame generated by the `generateSimulatedPersonsData` function.
...
#' Filters a persons data frame against a high risk group definition.
#'
#' @param persons                   A data frame generated by the `generateSimulatedPersonsData` function.
#' @param exposureCohort            A cohort against which to apply the risk criteria. This cohort is left-joined with persons.
#' @param highRiskGroupDefinition   A list of high risk group definitions. Each definition is a list with the following
#'                                  elements:
#'                                  - `var`: The variable to filter on. Must be one of `age_group_id`, `gender_concept_id`,
#'                                    `race_concept_id`, `ethnicity_concept_id`, `cohort_start_date`, or `cohort_end_date`
#'                                  - `operator`: The operator to use for the filter. Must be one of `=`, `==`, `<`, `<=`, `>`, `>=`,
#'                                    `in`, or `between`
#'
#'                                    Note: between requires a 2-item vector and checks the value is between, inclusive of endpoints.
#'                                  - `val`: The value to filter on. If `operator` is `in`, this should be a vector of
#'                                     values.
#' @param forCriteria               A value of `any` or `all` to determine whether risk group must meet all (`all`) criteria or
#'                                  any one of the criteria (`any`).
#'
#' @return
#' A vector of unique person IDs that match the provided group definition.
filterPersonsAgainstHighRiskGroupDefinition <- function(persons, exposureCohort, highRiskGroupDefinition, forCriteria = 'all') {
    ...
    for(i in 1:length(highRiskGroupDefinition)) {
        ...
        checkmate::assertChoice(item$var, c("age_group_id", "gender_concept_id", "race_concept_id", "ethnicity_concept_id", "cohort_start_date", "cohort_end_date"), add = errorMessages)
...
#'
#' @param targetCohort                      A data frame generated by the `getLatestCohortEntryFromEachPersonInTargetCohort` function.
#' @param includePersonIds                  A vector of person IDs to include in the outcomes cohort. If empty, all persons in the
#'                                          target cohort will be included.
#' @param outcomeId                         The ID of the outcome cohort definition. TODO: Multiple outcomes?
#' @param outcomeRate                       The proportion of persons in the comparator cohort who will experience the outcome.
#' @param riskWindowAfterTargetExposureDays The number of days after the target exposure that the adverse event outcome can occur.
#'
#' @return
#' A data frame containing the outcome cohort with the the following columns:
...
#'
#' @description
#' This takes the last exposure of the target cohort and generates outcomes for the comparator cohort. The outcome occurrence date
#' will be a random day between `washoutPeriodDays` days after the date of the target cohort start date (beginning of safe window)
#'  and `riskWindowAfterTargetExposureDays` days after the beginning of the safe window.
#'
#'
#' @param comparatorCohort                  A data frame generated by the `getLatestCohortEntryFromEachPersonInTargetCohort` function.
#' @param includePersonIds                  A vector of person IDs to include in the outcomes cohort. If empty, all persons in the
#'                                          comparator cohort will be included.
#' @param outcomeId                         The ID of the outcome cohort definition. TODO: Multiple outcomes?
#' @param outcomeRate                       The proportion of persons in the comparator cohort who will experience the outcome.
#' @param washoutPeriodDays                 The number of days after entering the target cohort until the person is considered no longer
...
generateOutcomesCohortForComparatorCohortFromLastExposureInTargetCohort <- function(
    ...
    data %>%
        ...
        mutate(
            cohort_definition_id = outcomeId,
            cohort_start_date = cohort_start_date + washoutPeriodDays + days_to_event, # Ensure start date is inside the safe window
            cohort_end_date = cohort_start_date + 1
        ) %>%
        select(-days_to_event)
}
...
#' Generate a target cohort from a persons data frame.
#'
#' @param persons                               A data frame generated by the `generateSimulatedPersonsData` function.
...
#'                                              at risk of adverse events caused by exposure to the vaccine.
#' @param studyEndDate                          The date the study ended in YYYY-MM-DD format.
#' @param targetDaysEligibleBeforeStudyEndDate  The number of days before the study end date that a person is eligible to enter the target cohort.
...
#'
#' @param targetCohort A data frame generated by the `generateTargetCohortFromPersons` function.
#'
#' @return
#' A data frame with the following columns:
#' - `cohort_definition_id`
...
#' the comparator cohort in the generated table since it does not correspond to any observed treatment or outcome, but rather
#' a window of time when members of the target cohort are no longer considered potentially at a higher risk for experiencing
#' adverse events from the vaccine. This functions allows for the specification of a low-risk and high-risk group of people who
#' are eligible for the target cohort. Adverse event outcomes are generated in proportion to the provided parameters for the low-
#' and high-risk groups during both the post-vaccine risk period and the safe period after the washout window concludes.
#'
#'
#' @param persons                                   A data frame with a format similar to the `persons` table in an OHDSI CDM instance.
#'                                                  The data frame must include the following columns: `person_id`, `birth_datetime`,
#'                                                  `gender_concept_id`, `race_concept_id`, and `ethnicity_concept_id`.
#' @param targetId                                  The ID of the target cohort that received the vaccine. This ID corresponds to the ID of
#'                                                  the cohort in the JSON cohort definition file.
#' @param outcomeIds                                A vector of IDs of the adverse event outcomes that are being studied. Each ID corresponds to the ID of
#'                                                  the cohort in the JSON cohort definition file. TODO: Support multiple outcomes
#' @param proportionSecondShot                      The proportion of the target cohort that received a second shot of the vaccine.
#' @param riskWindowAfterTargetExposureDays         The number of days after the target cohort received the vaccine that a person is considered
#'                                                  at risk of the outcome.
#' @param washoutPeriodDays                         The number of days after the target cohort received the vaccine after which they are considered
#'                                                  not to be at risk of the adverse event outcome being caused by the vaccine exposure.
#' @param highRiskGroupDefinition                   A list of items that define the high risk group. Each item is a list with the following elements:
#'                                                  - var: The name of the variable in the `persons` that is used to define the high risk group.
#'                                                         For example, "race_concept_id"
#'                                                  - operator: The operator to use to define the high risk group. This can be one of the following:
#'                                                              "in": The variable must be in a list of values.
#'                                                              "==": The variable must be equal to a value.
#'                                                              TODO:
#'                                                              >=
#'                                                              >
#'                                                              <=
...
#' @param lowRiskTargetIncidence                    The proportion of the low risk group to generate adverse events in the risk period.
#' @param lowRiskComparatorIncidence                The proportion of the low risk group to generate adverse events in the safe period.
#' @param highRiskTargetIncidence                   The proportion of the high risk group to generate adverse events in the risk period.
#' @param highRiskComparatorIncidence               The proportion of the high risk group to generate adverse events in the safe period.
#' @param studyEndDate                              The date on which the study ends.
#' @param targetDaysEligibleBeforeStudyEndDate      The number of days before the study end date that a person is eligible to be in the target cohort.
#'                                                  For example, if the study end date is 2021-06-30 and this value is 500, then the target cohort
#'                                                  can include people who received the vaccine between 2020-02-11 and 2021-06-30.
#'
#' @return
#' A data frame with the following columns:
#'
...
generateSimulatedCohortTable <- function(
    ...
    highRiskSubjectIds <- filterPersonsAgainstHighRiskGroupDefinition(persons, targetCohort, highRiskGroupDefinition, matchHighRiskCriteria)
    ...
    highRiskOutcomeCohortFromComparatorCohort <- generateOutcomesCohortForComparatorCohortFromLastExposureInTargetCohort(
        targetGroupIncludingOnlyLastCohortEntryPerPerson,
        includePersonIds = highRiskSubjectIds,
        outcomeId = outcomeIds[1],
...
generateSimulatedPersonsData <- function(n = 1000, studyEndDate = '2021-06-01') {
    matrix(
        ...
        dimnames = list(
            c(),
            c('person_id', 'birth_datetime', 'year_of_birth', 'gender_concept_id', 'race_concept_id', 'ethnicity_concept_id')
        )
    ) %>%
        as_tibble() %>%
        mutate(
...
#'
#' @return
#' A data frame with columns concept_id, name, domain, and probability.
#'
