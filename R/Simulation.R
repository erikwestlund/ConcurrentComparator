...
generateSimulatedPopulationAnalysisData <- function(
    ...
    checkmate::assertNumeric(timeAtRiskStartDays, lower = 1, add = errorMessages)
    checkmate::assertNumeric(timeAtRiskEndDays, lower = 1, add = errorMessages)
...
#' @param timeAtRiskEndDays The number of days after the target exposure to include in the risk window.
#'
#' @return
#' A tibble with the following columns:
...
generateTargetCohortFromSimulatedCohortTable <- function(
    ...
    checkmate::assertIntegerish(timeAtRiskEndDays, add = errorMessages)
    checkmate::reportAssertions(collection = errorMessages)
    ...
    cohort %>%
        ...
        mutate(
            cohort_end_date = as.Date(cohort_start_date) + timeAtRiskEndDays
        )
}
...
generateComparatorCohortFromSimulatedTargetCohortTable <- function(
    ...
    getLatestCohortEntryFromEachPersonInTargetCohort(targetCohort) %>%
        mutate(
            ...
            cohort_end_date = cohort_start_date + timeAtRiskEndDays
        )
}
...
#'                                          outcome.
#' @param timeAtRiskEndDays The number of days after the target exposure that the adverse event outcome
#'                                          can occur.
#'
#' @return
...
generateOutcomesCohortFromTargetCohort <- function(
    ...
    data %>%
        mutate(
            days_to_event = sample(1:(timeAtRiskEndDays - 1), nrow(data), replace = TRUE)
        ) %>%
...
#' (beginning of safe window) and `timeAtRiskEndDays` days after the beginning of the safe window.
#'
#'
...
#' @param timeAtRiskEndDays The number of days after entering the target cohort when a person is
...
#'                                                  the vaccine.
#' @param timeAtRiskEndDays         The number of days after the target cohort received the vaccine that
...
generateSimulatedCohortTable <- function(
        ...
        timeAtRiskEndDays = 21,
        washoutPeriodDays = 22,
        highRiskGroupDefinition = list(), # By default, there is no high risk group
    ...
    checkmate::assertIntegerish(timeAtRiskEndDays, len = 1, add = errorMessages)
...
#' Generate a simulated observation period table consistent with the OMOP CDM.
#'
#' @param cohort                                A tibble generated by `generateSimulatedCohortTable`.
#' @param washoutPeriod                         The number of days after the cohort start date until the person is
#'                                              considered no longer at risk of adverse events caused by exposure.
#'
#' @return
#' A tibble with the following columns:
...
#'
#' @export
generateSimulatedObservationPeriodTable <- function(
  cohort,
  washoutPeriod = 21
) {
    cohort %>%
        group_by(subject_id) %>%
        summarise(
            observation_period_start_date = min(cohort_start_date) - 1, # observe one day before the cohort start date
            observation_period_end_date = max(cohort_end_date) + 1 + 2*washoutPeriod  # want to cover washout period of target and comparator
        ) %>%
        ungroup() %>%
        mutate(
            observation_period_id = row_number(),
            person_id = subject_id,
        ) %>%
        select(observation_period_id, person_id, observation_period_start_date, observation_period_end_date)
}
...
#'
#' @export
generateSimulatedPersonsData <- function(n = 1000, studyEndDate = '2021-06-30') {
    matrix(
        nrow = n,
        ncol = 6,
        dimnames = list(
...
#' Generate a simulated condition era table from a cohort table and set of outcome IDs.
#'
#' @param cohort        A tibble generated by the `generateSimulatedCohortTable` function.
#' @param outcomeIds    A vector of IDs of the adverse event outcomes that are being studied.
#'
#' @return
#' A tibble.
generateSimulatedConditionEraTable <- function(cohort, outcomeIds) {
  cohort %>%
    filter(cohort_definition_id %in% outcomeIds) %>%
    mutate(
      condition_era_id = row_number(),
      person_id = subject_id,
      condition_concept_id = cohort_definition_id,
...
#' Generate a simulated concept ancestor table from a condition era table.
#'
#' @param conditionEraTable     A tibble generated by the `generateSimulatedConditionEraTable` function.
#'
#' @return
#' A tibble.
generateSimulatedConceptAncestorTable <- function(conditionEraTable) {
    conditionEraTable %>%
